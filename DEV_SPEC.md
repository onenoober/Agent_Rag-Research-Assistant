...
> **核心定位：模块化、可扩展的 RAG 系统**

本项目基于以下核心设计理念构建：

- **模块化架构**：采用插件化设计，LLM、Embedding、Reranker、Splitter、VectorStore 等核心组件均定义抽象接口，支持灵活替换与扩展。
- **可观测性**：内置完整的追踪系统，覆盖 Ingestion 和 Query 全链路，支持 JSON Lines 持久化存储。
- **开箱即用**：提供 MCP 标准接口，可直接对接 Copilot、Claude 等 AI 工具。
- **生产级质量**：三层测试体系（Unit/Integration/E2E）确保代码质量与系统稳定性。
...
- **M6（完成阶段 H+I）**：评估体系完整 + E2E 验收通过 + 文档完善，形成可复现的生产级项目。
...
本项目的架构设计完全支持向云端迁移，为企业级部署提供完整的技术路径：

- **Server 容器化**：编写 Dockerfile，将 MCP Server 打包为容器，实现 Python 环境隔离、依赖管理及 Docker 最佳实践。
- **云端接入**：支持部署至 Azure Container Apps 或 AWS Lambda。
    - **能力增强**：处理网络延时、配置 API Gateway、增加 AuthN/AuthZ 鉴权机制。
- **多租户与并发**：支持团队共享服务。
    - **实现要点**：在 Chroma 中实现 Namespace 隔离、处理并发请求锁、优化 embedding 缓存策略。
