...
class ConcurrencySettings:
    """Settings for controlling parallel processing across pipeline components.
    
    Attributes:
        llm_transform: Max parallel workers for LLM-based transforms
                      (chunk_refiner, metadata_enricher). Default: 5
        image_caption: Max parallel workers for image captioning.
                      Lower default due to API cost. Default: 3
        embedding_batch: Workers for embedding batch processing.
                        1 = sequential dense then sparse
                        2+ = parallel dense and sparse encoding. Default: 1
    """
    llm_transform: int = 5
    image_caption: int = 3
    embedding_batch: int = 1
...
class IngestionSettings:
    ...
    formula_extraction: Optional[Dict[str, Any]] = None  # 动态配置
    concurrency: Optional[ConcurrencySettings] = None  # 并发配置
...
class Settings:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Settings":
        ...
        if "ingestion" in data:
            ...
            # Parse concurrency settings if present
            concurrency_config = None
            if "concurrency" in ingestion:
                conc = ingestion.get("concurrency", {})
                concurrency_config = ConcurrencySettings(
                    llm_transform=conc.get("llm_transform", 5),
                    image_caption=conc.get("image_caption", 3),
                    embedding_batch=conc.get("embedding_batch", 1),
                )
            ...
            ingestion_settings = IngestionSettings(
                ...
                formula_extraction=ingestion.get("formula_extraction"),  # 可选配置
                concurrency=concurrency_config,  # 并发配置
            )
