...
def _render_split_stage(data: Dict[str, Any], *, trace_idx: int = 0) -> None:
    ...
    if chunks:
        ...
        # Use selectbox instead of nested expander to avoid Streamlit nesting restriction
        chunk_options = [f"Chunk #{i+1} — {chunk.get('chunk_id', '')[:20]} — {chunk.get('char_len', 0)} chars" for i, chunk in enumerate(chunks)]
        selected_idx = st.selectbox("Select chunk to view", options=list(range(len(chunk_options))), format_func=lambda i: chunk_options[i], key=f"split_select_{trace_idx}")
        if selected_idx is not None and 0 <= selected_idx < len(chunks):
            chunk = chunks[selected_idx]
            text = chunk.get("text", "")
            st.text_area(
                f"split_{selected_idx}",
                value=text,
                height=max(100, min(len(text) // 2, 500)),
                disabled=True,
                label_visibility="collapsed",
                key=f"split_{trace_idx}_{selected_idx}",
            )
    else:
...
def _render_transform_stage(data: Dict[str, Any], *, trace_idx: int = 0) -> None:
    ...
    if chunks:
        ...
        for i, chunk in enumerate(chunks):
            ...
            chunk_options.append(f"Chunk #{i+1} — {chunk_id[:20]} — {badges}")
        
        selected_idx = st.selectbox("Select chunk to view", options=list(range(len(chunk_options))), format_func=lambda i: chunk_options[i], key=f"transform_select_{trace_idx}")
        if selected_idx is not None and 0 <= selected_idx < len(chunks):
            chunk = chunks[selected_idx]
            ...
            if text_before or text_after:
                ...
                with col_before:
                    ...
                    st.text_area(
                        f"before_{selected_idx}",
                        value=text_before if text_before else "(empty)",
                        height=_h,
                        disabled=True,
                        label_visibility="collapsed",
                        key=f"transform_before_{trace_idx}_{selected_idx}",
                    )
                with col_after:
                    st.markdown("*After refinement + enrichment:*")
                    st.text_area(
                        f"after_{selected_idx}",
                        value=text_after if text_after else "(empty)",
                        height=_h,
                        disabled=True,
                        label_visibility="collapsed",
                        key=f"transform_after_{trace_idx}_{selected_idx}",
                    )
    else:
...
def _render_embed_stage(data: Dict[str, Any]) -> None:
    ...
    # ── Dense encoding table (outside tabs to avoid expander nesting) ──
    st.markdown("### 🟦 Dense Encoding")
    ...
    # ── Sparse encoding table with top terms ──
    st.markdown("### 🟨 Sparse Encoding (BM25)")
    ...
    # Top terms: use selectbox instead of nested expander
    has_top_terms = any(chunk.get("top_terms", []) for chunk in chunks)
    if has_top_terms:
        st.markdown("**Top Terms per Chunk**")
        # Build options for chunks that have top terms
        top_term_options = [
            f"Chunk #{i+1} — {chunk.get('chunk_id', '')[:20]}"
            for i, chunk in enumerate(chunks)
            if chunk.get("top_terms", [])
        ]
        if top_term_options:
            # Get indices of chunks with top terms
            chunk_indices = [i for i, chunk in enumerate(chunks) if chunk.get("top_terms", [])]
            selected = st.selectbox("Select chunk to view top terms", options=list(range(len(chunk_indices))), format_func=lambda i: top_term_options[i])
            if selected is not None:
                chunk_idx = chunk_indices[selected]
...
def _render_upsert_stage(data: Dict[str, Any]) -> None:
    ...
    # ── Dense store details ──
    if dense_store:
        st.markdown("### 🟦 Dense Vector Store (ChromaDB)")
    ...
    if image_store and image_store.get("count", 0) > 0:
        st.markdown(f"### 🖼️ Image Storage ({image_store.get('count', 0)} images)")
    ...
    if chunk_mapping:
        st.markdown(f"### 🔗 Chunk → Vector Mapping ({len(chunk_mapping)} entries)")
        mapping_rows = [
            {
                "#": i + 1,
