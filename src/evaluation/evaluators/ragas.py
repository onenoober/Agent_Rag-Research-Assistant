...
class RagasEvaluator(BaseEvaluator):
    ...
    async def evaluate(
        ...
        **kwargs: Any,
        ...
        try:
            result = await self._run_ragas(query, contexts, generated_answer)
    ...
    async def _run_ragas(
        ...
        """Execute Ragas async metrics and return normalised scores.

        Ragas 0.4+ collections metrics use async ``ascore()`` instead of
        the legacy ``score()`` pipeline.  Each metric has its own signature:
        ...
        for metric_name in self._metric_names:
            ...
            # Handle different Ragas version return types robustly
    ...
    def _normalize_score(result: Any) -> float:
        """Normalize ragas score result to float.

        Handles different Ragas version return types:
        - Score objects with .value attribute (ragas >= 0.4)
        - Direct float/int values
        - numpy.float64 and other numeric types
        - DataFrame rows (for legacy ragas versions)
        """
        import pandas as pd
        import numpy as np

        # Already a native float
        if isinstance(result, float):
            return result
        # Already a native int
        if isinstance(result, int):
            return float(result)
        # numpy numeric types
        if isinstance(result, (np.floating, np.integer)):
            return float(result)
        # DataFrame row or similar (legacy ragas behavior)
        if isinstance(result, pd.Series):
            # Get first value from series
            return float(result.iloc[0]) if len(result) > 0 else 0.0
        if isinstance(result, pd.DataFrame):
            # Get first row, first column value
            if not result.empty:
                return float(result.iloc[0, 0])
            return 0.0
        # Score object with .value attribute
        ...
        try:
            ...
            logger.warning("Unable to convert ragas result to float: %s (type: %s)", result, type(result).__name__)
            return 0.0
    ...
    def _build_wrappers(self) -> tuple:
        ...
        # Build extra kwargs for llm_factory
        model_name = getattr(llm_cfg, "model", "").lower()
        ...
        llm = llm_factory(llm_cfg.model, client=llm_client, **llm_kwargs)
