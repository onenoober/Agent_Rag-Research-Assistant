...
|- Sequential Encoding: Dense and sparse encoding run sequentially for reliability
"""
...
class BatchProcessor:
    ...
    - Sequential Encoding: Dense then sparse for each batch (reliable)
    ...
    def process(
        ...
        3. Process each batch: dense encoding first, then sparse encoding
        ...
        # Process all batches (sequential: dense then sparse for each batch)
        dense_vectors: List[List[float]] = []
        sparse_stats: List[Dict[str, Any]] = []
        successful_chunks = 0
        failed_chunks = 0
        ...
        for batch_idx, batch in enumerate(batches):
            ...
            try:
                # Sequential encoding: dense first, then sparse
                # Note: Parallel encoding was attempted but caused issues with
                # embedding provider (race conditions, API throttling). Sequential mode
                # is more reliable and ensures order preservation.
                ...
                logger.error(f"  Batch {batch_idx} failed: {type(e).__name__}: {e}")
                failed_chunks += len(batch)
                if trace:
                    trace.record_stage(
                        f"batch_{batch_idx}_error",
                        {"error": str(e), "error_type": type(e).__name__, "batch_size": len(batch)}
                    )
