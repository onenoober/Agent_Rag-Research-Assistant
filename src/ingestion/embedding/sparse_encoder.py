...
class SparseEncoder:
    ...
    def encode(
        ...
        for i, chunk in enumerate(chunks):
            ...
            # Build enhanced text including:
            # 1. Image captions for BM25 term matching on visual content
            # 2. LaTeX formulas for mathematical term matching
            ...
            if chunk.metadata:
                ...
                if captions:
                    ...
                    if caption_texts:
                        enhanced_parts.extend(caption_texts)
                ...
                if formulas:
                    for f in formulas:
                        latex = f.get('latex', '')
                        if latex:
                            # Extract alphanumeric terms from LaTeX for BM25
                            # Simple extraction: get command names from \command{...} patterns
                            try:
                                # Extract command names like frac, sum, integral, etc.
                                clean_latex = re.sub(r'\\([a-zA-Z]+)\{[^}]*\}', r' \1 ', latex)
                                clean_latex = re.sub(r'[{}^_]', ' ', clean_latex)
                                clean_latex = re.sub(r'\s+', ' ', clean_latex).strip()
                            except re.error:
                                # Fallback: just remove special characters
                                clean_latex = re.sub(r'[{}^_\\]', ' ', latex)
                            if clean_latex.strip():
                                enhanced_parts.append(clean_latex)
                
                # Add title and summary for term matching
            ...
            terms = self._tokenize(enhanced_text)
