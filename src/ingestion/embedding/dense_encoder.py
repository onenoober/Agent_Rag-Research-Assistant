...
class DenseEncoder:
    ...
    def encode(
        ...
        # Extract and enhance text from chunks
        # Include metadata enrichment for richer embeddings:
        # - image_captions: Image descriptions for visual content
        # - formulas: LaTeX formulas for mathematical content
        # - title/summary/tags: LLM-generated semantic metadata
        texts = []
        for chunk in chunks:
            text_parts = [chunk.text]
            
            # 1. Append image captions if present
            if chunk.metadata and "image_captions" in chunk.metadata:
                captions = chunk.metadata["image_captions"]
                if captions:
                    caption_texts = [cap.get('caption', '') for cap in captions if cap.get('caption')]
                    if caption_texts:
                        text_parts.append("Images: " + " ".join(caption_texts))
            
            # 2. Append LaTeX formulas if present (for mathematical content)
            if chunk.metadata and "formulas" in chunk.metadata:
                ...
                if formulas:
                    ...
                    if latex_parts:
                        text_parts.append("Formulas: " + " ".join(latex_parts))
            
            # 3. Append LLM-generated metadata if present (title, summary)
            # These are generated by MetadataEnricher and contain semantic understanding
            if chunk.metadata:
        ...
        logger.debug(f"DenseEncoder.encode(): {len(texts)} texts to encode")
        for i, t in enumerate(texts):
            logger.debug(f"  Text {i}: {t[:100]}...")
        ...
        for batch_start in range(0, len(texts), self.batch_size):
            ...
            try:
                ...
                logger.debug(f"  Batch {batch_start}-{batch_end}: got {len(batch_vectors)} vectors")
                
            except Exception as e:
                # Re-raise with context about which batch failed
                logger.error(f"DenseEncoder failed at batch {batch_start}-{batch_end}: {e}")
                raise RuntimeError(
        ...
        logger.debug(f"DenseEncoder.encode(): returning {len(all_vectors)} vectors")
        return all_vectors
