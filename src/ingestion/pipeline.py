...
document ingestion flow:
    ...
    2. Document Loading (Universal loader for PDF, DOCX, etc.)
...
- Multi-Format: Supports PDF, DOCX, TXT, MD via UniversalLoader
"""
...
class IngestionPipeline:
    ...
    - Document loading (Universal loader for PDF, DOCX, TXT, MD)
    ...
    - Dense embedding (Qwen/OpenAI embeddings)
    ...
    def __init__(
        ...
        # Stage 2: Loader (supports PDF, DOCX, TXT, MD)
        ...
        logger.info("  + UniversalLoader initialized (PDF, DOCX, TXT, MD)")
        ...
        # Formula Extractor (uses Pix2Text)
        formula_config = settings.ingestion.formula_extraction if settings.ingestion else None
        formula_enabled = formula_config.get('enabled', True) if formula_config else True
        formula_device = formula_config.get('device', 'cpu') if formula_config else 'cpu'
        formula_language = formula_config.get('language', 'en') if formula_config else 'en'

        self.formula_extractor = FormulaExtractor(
            settings,
            device=formula_device,
            language=formula_language
        )
        logger.info(f"  + FormulaExtractor initialized (enabled={formula_enabled}, device={formula_device})")
        ...
        # BatchProcessor: processes chunks through both encoders sequentially
    ...
    def run(
        ...
        Args:
            file_path: Path to the file to process (PDF, DOCX, TXT, MD)
            trace: Optional trace context for observability
            on_progress: Optional callback (stage_name, current, total)
                invoked when each pipeline stage completes.
        ...
        try:
            ...
            if not self.force and self.integrity_checker.should_skip(file_hash):
                logger.info(f"  + File already processed, skipping (use force=True to reprocess)")
            ...
            # Stage 3: Chunking
            logger.info("[Stage 3] Document Chunking")
            _notify("split", 3)
            ...
            # 4d: Formula Extraction (using Pix2Text)
            logger.info("  4d. Formula Extraction...")
            chunks = self.formula_extractor.transform(chunks, trace)
            extracted_formulas = sum(1 for c in chunks if c.metadata.get("formulas"))
            logger.info(f"      Chunks with formulas: {extracted_formulas}")
            ...
            stages["transform"] = {
                ...
                "formula_extractor": {"chunks_with_formulas": extracted_formulas}
            }
            ...
            try:
                ...
                logger.error(f"BatchProcessor.process() failed: {type(e).__name__}: {e}", exc_info=True)
                raise
            ...
            if batch_result.failed_chunks > 0:
                logger.warning(f"  + Failed chunks: {batch_result.failed_chunks}/{len(chunks)}")
            ...
            vector_ids = self.vector_upserter.upsert(chunks, dense_vectors, file_hash, trace)
            ...
            # 6b: BM25 Index
            bm25_doc_id = vector_ids[0].split('_')[0] if vector_ids else file_hash[:8]
            logger.info(f"  6b. BM25 Index (doc_id prefix: {bm25_doc_id})...")
