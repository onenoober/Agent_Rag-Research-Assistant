...
class DocumentManager:
    ...
    def delete_document(
        ...
        # Note: BM25 stores chunk_id as vector_id which starts with source_path hash (8 chars)
        # We need to compute the same 8-char hash used during storage
        try:
            # Compute the same hash that was used to generate chunk_id in vector_upserter
            # chunk_id format: {source_hash}_{index:04d}_{content_hash}
            # where source_hash = sha256(source_path)[:8]
            source_hash_for_bm25 = hashlib.sha256(source_path.encode("utf-8")).hexdigest()[:8]
            result.bm25_removed = self.bm25.remove_document(
                source_hash_for_bm25, collection
            )
        except Exception as e:
