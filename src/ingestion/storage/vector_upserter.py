...
class VectorUpserter:
    ...
    def upsert(
        ...
        Args:
            ...
            file_hash: Optional document hash for deletion/filtering. If provided,
                will be stored in metadata as 'doc_hash' for later deletion.
        ...
        for chunk, vector in zip(chunks, vectors):
            ...
            # Note: chunk.metadata may not have doc_hash, so we add it explicitly
            # This is critical for document deletion to work properly
            record_metadata = {
                **chunk.metadata,  # Preserve all original metadata
                "chunk_id": chunk_id,  # Keep for reference
            }
            
            # P1-4: Do NOT store text in metadata - ChromaDB stores it in 'documents' field
            # This reduces storage overhead significantly
            # Text can be retrieved from the 'documents' field during queries
            
            # Add doc_hash for document-level deletion if provided
            if file_hash:
                record_metadata["doc_hash"] = file_hash
            
            record = {
                "id": chunk_id,
                "vector": vector,
                "metadata": record_metadata,
                # P1-4: ChromaStore.upsert() extracts text from metadata['text'] to 'documents' field
                # We need to add it here for ChromaDB to store properly
                "text": chunk.text,  # Used by ChromaStore to populate documents field
            }
