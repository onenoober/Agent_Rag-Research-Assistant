...
# Legacy constant for backward compatibility (overridden by settings)
_DEFAULT_MAX_WORKERS = 3


def _get_default_max_workers() -> int:
    """Get default max workers from settings, with fallback to legacy constant."""
    try:
        settings = load_settings()
        if settings.ingestion and settings.ingestion.concurrency:
            return settings.ingestion.concurrency.image_caption
    except Exception:
        pass
...
class ImageCaptioner(BaseTransform):
    ...
    def __init__(
        ...
        Args:
            ...
            llm: Optional Vision LLM instance (for testing; auto-created if None)
            max_workers: Max parallel workers for caption generation.
                        If None, uses settings.ingestion.concurrency.image_caption
        """
        ...
        elif settings.ingestion and settings.ingestion.concurrency:
            self._max_workers = settings.ingestion.concurrency.image_caption
        else:
            self._max_workers = _get_default_max_workers()
    ...
    def _generate_captions_parallel(
        ...
        max_workers = min(self._max_workers, len(images_to_caption))
