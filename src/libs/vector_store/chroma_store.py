...
# Performance tuning: Limit BLAS library threads to prevent memory allocation failures
# Critical for Windows/OpenBLAS environments with limited memory
os.environ.setdefault("OPENBLAS_NUM_THREADS", "1")
os.environ.setdefault("OMP_NUM_THREADS", "1")
os.environ.setdefault("MKL_NUM_THREADS", "1")

# Disable ChromaDB's automatic embedding model loading
# Since we use external embedding providers (OpenAI/Qwen/etc.),
# ChromaDB's default ONNX embedding model is not needed and causes issues
os.environ.setdefault("CHROMADB_DISABLE_AUTO_EMBED", "true")
    ...
    def upsert(
        ...
        **kwargs: Any,
        ...
        for record in records:
            ...
            # P1-4: Remove 'text' from metadata to avoid redundancy
            metadata = {k: v for k, v in record.get('metadata', {}).items() if k != 'text'}
            ...
            # Document: P1-4 - extract text from record['text'] or record['metadata']['text']
            # This avoids storing text redundantly in both places
            document = record.get('text') or metadata.get('text', record['id'])
    ...
    def _sanitize_metadata(self, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in metadata.items():
            if isinstance(value, (str, int, float, bool)):
                ...
                # Convert dict to JSON string for proper storage
